...
class AVLTree
  ...
  def remove_unbounded(value)
    done = false @root, done = AVLTree.remove_unbounded!(@root, value, done)
    true
  end
  ...
  def self.insert!(node, value, done)
    return AVLTreeNode.new(value) if node == nil
  ...
  def self.adjust_balance!(root, dir, bal)
    other_dir = dir == 0 ? 1:0
    r = root.subtrees[dir]
    rl = n.subtrees[other_dir]

    if rl.balance == 0
      root.balance = 0
      r.balance = 0
    elsif rl.balance == bal
      root.balance = -bal
      r.balance = 0
    else
      root.balance = 0
  ...
  def self.single_rotation_unbounded!(root, dir)
    ...
    new_root_balance = [new_root_left_height, root.balance].max + 1
    new_root
   end
   ...
   def self.insert_unbounded!(root, value, done)
     return [AVLTreeNode.new(value), false] if root == nil
     dir = value < root.value ? 0:1
     other_dir = dir == 0 ? 1:0
